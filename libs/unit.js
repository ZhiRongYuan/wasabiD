...
baseUtil.getSource=function(data,source) {
    ...
    if(returnData.length!=0){
        if(returnData[0] instanceof Object){}else{//当得到的数据 不是数组对象时,做一个简单处理,如["广百仓"]=>[{text:"广百仓",value:"广百仓"}]
            for(let i=0;i<returnData.length;i++){
                returnData[i]={text:returnData[i],value:returnData[i]};
            }
        }
    }
    return  returnData;
...
baseUtil.buffMove=function(obj,json,fn) {//缓冲运动
    clearInterval(obj.timer);
    var iSpeed;
    var stop=true;
    obj.timer=setInterval(function () {
        for(var key in json){
            var cur=parseInt(baseUtil.getStyle(obj,key));
            iSpeed=(json[key]-cur)/8;
            if(cur+iSpeed!=json[key]){//所以都到达目标值
                stop=false;
            }
            obj.style[key]=cur+iSpeed+"px";
        }
...
var elasticMoveLeft=0;
var elasticMoveiSpeed=0;//速度必须放外面，否则就不是每次的执行结果了
baseUtil.elasticMove=function (obj,attr,target) {//弹性运动
    clearInterval(obj.timer);
    obj.timer=setInterval(function () {
        var cur=parseInt(baseUtil.getStyle(obj,attr));
        elasticMoveiSpeed+=(target-cur)/5;
        elasticMoveiSpeed*=0.7;
        elasticMoveLeft+=elasticMoveiSpeed;
        //obj.style[attr]=cur+elasticMoveiSpeed+"px";//这种写法有误差，某一个li处会多或少一块，所以改成自定义变量left
        if (Math.abs(elasticMoveiSpeed)<1 && Math.abs(elasticMoveLeft-target)<1) {
            //判断条件就是距离足够近，速度足够小，利用绝对值
            clearInterval(obj.timer);
            obj.style[attr]=target+'px';
            //以防万一，使定时器关闭时就卡在对应位置
        }else{
            obj.style[attr]=elasticMoveLeft+'px';
        }
    },30)
}

baseUtil.getStyle=function (obj,attr) {//获取属性值

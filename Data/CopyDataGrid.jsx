...
var DataGrid=React.createClass({
    ...
    propTypes: {
        ...
        cellMaxWidth:React.PropTypes.number,//单元格最大宽度  超过最大宽度则换行
        ...
        defaultHeaderUrl:React.PropTypes.string,//默认的自定义表头接口
                ...
                if (this.state.headerMenu.length > 0 && this.state.headerMenu.indexOf(header.label) > -1) {//父组件更新状态值，发现某一行处理被隐藏中，则不显示
                    ...
                    headers.push(
                        <th key={"header" + index.toString()} name={header.label} {...props}
                            ...
                            }}><span>{header.label}</span>{savecontrol}</div>
                        </th>)
    ...
    renderBody:function(fixed) {//渲染表体
        ...
        this.state.data.map((rowData,rowIndex)=> {
                    ...
                    if (columnIndex==0&&this.props.detailAble) {
                        ...
                        tds.push(<td onClick={this.detailHandler.bind(this,rowIndex,rowData)}
                            ...
                            <div className="wasabi-table-cell"  style={{width:(header.width?header.width:null),textAlign:(header.align?header.align:"left")}}>
                                <div> <span style={{display:"inline-block"}}>{content}</span><LinkButton style={{position:"relative",top:"2px"}} iconCls={iconCls} color="#666666" tip="查看详情"></LinkButton></div>
                            </div>
                        </td>);
                    }
                    else {
            ...
            if(this.state.detailIndex==key&&tds.length!=0)//如果左右固定栏  没有数据  则详情页不显示了
            {
                ...
                switch (fixed){
                    case "fixedLeft":
                        trobj.push(<tr ref="fixedLeftDetail" key={this.state.detailIndex+"detail"} style={{borderLeft:"1px solid #0c4144"}}>
                            <td colSpan={colSpan}><div className="wasabi-detail" ><div className="wasabi-detail-Wrap">{this.state.detailView}</div></div></td>
                        </tr>);
                        break;
                    case "fixedRight":
                        trobj.push(<tr ref="fixedRightDetail" key={this.state.detailIndex+"detail"} style={{borderLeft:"1px solid #0c4144"}}>
                            <td colSpan={colSpan}><div className="wasabi-detail" ><div className="wasabi-detail-Wrap">{this.state.detailView}</div></div></td>
                        </tr>);
                        break;
                    default:
                        trobj.push(<tr ref="realTableDetail" key={this.state.detailIndex+"detail"} style={{borderLeft:"1px solid #0c4144"}}>
